...
export default function URLTool() {
  ...
  useEffect(() => {
    const qText = searchParams.get('text') || searchParams.get('url') || searchParams.get('input');
    ...
    if (qText !== null && qText !== undefined) {
      setInputText(qText);
      if (encode) {
        setOutputText(encodeURIComponent(qText));
      } else {
        try {
          setOutputText(decodeURIComponent(qText));
        } catch {
          setOutputText('');
        }
      }
  ...
  const handleInputChange = (event) => {
    ...
    if (isEncodeMode) {
      setOutputText(encodeURIComponent(newText));
    } else {
      try {
        setOutputText(decodeURIComponent(newText));
      } catch (error) {
        setOutputText('');
      }
    }
