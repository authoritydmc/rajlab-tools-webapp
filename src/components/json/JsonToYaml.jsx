    ...
    <ToolPageLayout title="JSON to YAML" icon={<FaExchangeAlt />} siblings={siblings} currentPath="/json-to-yaml" breadcrumb={[{label: 'JSON Utilities', path: '/json-viewer'}]}>
      ...
      <div className={`w-full mx-auto p-6 shadow-lg rounded-md ${isDarkMode ? 'bg-slate-900/60 border-slate-700/50 backdrop-blur-xl' : 'bg-white/60 border-slate-200/50 backdrop-blur-xl'} border`}>
        ...
        {output && (
          <>
            <div className="mb-4">
              <label className="block font-bold mb-2">YAML Output</label>
            ...
            <div className="flex gap-2 justify-center">
              ...
              <button onClick={handleDownload} className={`p-2 rounded-md transition-colors duration-300 ${isDarkMode ? 'bg-purple-600 text-white hover:bg-purple-700' : 'bg-purple-500 text-white hover:bg-purple-600'}`}><FaDownload className="inline mr-1" />Download YAML</button>
            </div>
          </>
        )}
      </div>
